...
import { useRecentRecords } from "../hooks/useDashboardData";
...
export interface RecentRecordsTableProps {
  batchId: number | null;
}
...
export function RecentRecordsTable({ batchId }: RecentRecordsTableProps) {
  ...
  const columns = useMemo<ColumnDef<RecentRecordRow>[]>(
    () => [
      {
      ...
      },
      {
        ...
        cell: (info) => {
          const v = info.getValue<number | null>();
          return (
            <span className={`tabular-nums ${v && v > 0 ? "text-oee-low" : ""}`}>
              {fmtNum(v)}
            </span>
          );
        },
      ...
      <table className="w-full text-sm">
        <thead>
          {table.getHeaderGroups().map((hg) => (
            <tr key={hg.id} className="border-b bg-muted/40 text-left text-xs uppercase text-muted-foreground">
              {hg.headers.map((h) => (
                <th
                  key={h.id}
